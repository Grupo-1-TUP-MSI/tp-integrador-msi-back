...
const createCliente = async (req, res) => {
  ...
  if (
    (idtipodocumento == 0,
    documento == 0,
    tipoiva == 0 || nombre == "" || direccion == "" || cp == "" || email == "")
  ) {
    return res.status(400).json({
      mensaje: "Debe cargar todos los campos requeridos",
      status: 400,
    });
  }
  try {
...
const updateCliente = async (req, res) => {
  ...
  try {
    ...
    if (!clienteFound) {
      return res
        .status(400)
        .json({ mensaje: "El cliente no existe", status: 400 });
    }
    ...
    res
      .status(200)
      .json({ mensaje: "Cliente actualizado correctamente", status: 200 });
  } catch (error) {
    console.log(error);
    res
      .status(400)
...
export { getClientes, createCliente, deleteCliente, updateCliente, getCliente };
